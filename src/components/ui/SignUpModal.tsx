...
export function SignUpModal({ syncUserState, closeModal }) {
    ...
    function writeToStorageHandler(event) {
        ...
        console.log(event.target.signUpUsername.value, event.target.signUpPassword.value)
        const formData = {
        ...
        <>
            <div className="py-10 bg-gray-900 text-4xl transition duration-150 ease-in-out z-10 fixed top-0 right-0 bottom-0 left-0" id="modal">
                <div role="alert" className="container mx-auto w-11/12 md:w-8/10 max-w-lg">
                    <form className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400" onSubmit={writeToStorageHandler}>
